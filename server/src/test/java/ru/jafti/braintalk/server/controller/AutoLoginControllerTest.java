package ru.jafti.braintalk.server.controller;

import org.junit.jupiter.api.Test;
//import ru.jafti.braintalk.server.RendezvousPoint;
//import ru.jafti.braintalk.server.connection.Session;
//import ru.jafti.braintalk.server.service.TalkerProfileRepository;

import java.util.UUID;

import static org.mockito.Mockito.*;

public class AutoLoginControllerTest {
        @Test
        void parseTest() {
//            RendezvousPoint rendezvousPoint = mock(RendezvousPoint.class);
//            Session session = mock(Session.class);
//            TalkerProfileRepository talkerProfileService = mock(TalkerProfileRepository.class);
//            UUID testId = UUID.fromString("11111111-1d13-4c32-95d9-18be5c07dc8b");
//            when(talkerProfileService.findById(testId)).thenReturn("Joe");
//            AutoLoginController uut = new AutoLoginController(talkerProfileService, rendezvousPoint);
//
//            uut.apply("/auto-login 11111111-1d13-4c32-95d9-18be5c07dc8b", session);
//            verify(rendezvousPoint).goIn("Joe", session);
//            verify(session).setLoggedIn("Joe", testId);
        }
}
